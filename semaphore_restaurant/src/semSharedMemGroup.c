...
static void checkInAtReception(int id)
{
    // TODO receptionistRequestPossible -- bloqueia grupo até ser possível fazer um request ao receptionist
    ...
    // TODO receptionistReq -- permite ao receptionist receber o pedido
    ...
    // waitForTable -- bloqueia até ter mesa
...
static void orderFood (int id)
{
    // TODO waiterRequestPossible -- bloqueia grupo até ser possivel fazer o request ao waiter
    ...
    // TODO waiterRequest -- permite ao waiter receber o request
...
static void waitFood (int id)
{
    ...
    // Espera que o waiter traga a comida
...
static void checkOutAtReception (int id)
{
    // TODO receptionistRequestPossible -- grupo espera que o receptionist esteja disponivel
    if (semDown (semgid, sh->receptionistRequestPossible) == -1)
    {
        perror ("error on the up operation for semaphore access (PT)");
        exit (EXIT_FAILURE);
    ...
    // TODO receptionistReq -- avisa receptionist que quer a conta
    if (semUp (semgid, sh->receptionistReq) == -1)
    {
        perror ("error on the up operation for semaphore access (PT)");
        exit (EXIT_FAILURE);
    ...
    // Espera que o receptionist acabe o pagamento
